...
public class DatabaseHandler extends SQLiteOpenHelper
{
	...
	public void addMealEntry(MealEntry mealEntry)
	{
		...
		Log.i("(DB) Food consumed for " + mealEntry.getMealType() + " at "
				+ mealEntry.getDateStr(), foodTitleStr);
		Log.i("New Meal Entry added: ", mealEntry.getMealType() + " at "
	...
	public MealEntry getMealEntry(String mealType, String dateStr)
	{
		...
		if (cursor.getCount() > 0)
		{
			...
			List<String> foodTitleSplitStr = Arrays.asList(foodTitleStr
					.split("\\s*,\\s*"));

			String numOfEachFoodStr = cursor.getString(4);
			List<String> numOfEachFoodSplitStr = Arrays.asList(numOfEachFoodStr
					.split("\\s*,\\s*"));
	...
	public List<MealEntry> getAllMealEntry()
	{
		...
		if (cursor.moveToFirst())
		{
			do
			{
				...
				mealEntry.setMealType(cursor.getString(0));
				mealEntry.setDateStr(cursor.getString(1));
				mealEntry.setTotalCalorie(Double.parseDouble(cursor
						.getString(2)));
				...
				String numOfEachFoodStr = cursor.getString(4);
				List<String> numOfEachFoodSplitStr = Arrays
						.asList(numOfEachFoodStr.split("\\s*,\\s*"));
	...
	public Pedometer getPedometerState(String dateStr)
	{
		...
		Cursor cursor = db.query(TABLE_PEDOMETER,
				new String[] { KEY_PEDOMETER_DATE, KEY_PEDOMETER_STEP,
						KEY_PEDOMETER_DISTANCE }, KEY_PEDOMETER_DATE + "=?",
	...
	public void updatePedometerState(Pedometer pedo)
	{
		...
		db.update(TABLE_PEDOMETER, values, KEY_PEDOMETER_DATE + "=?",
