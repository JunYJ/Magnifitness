				...
				Request meRequest = Request.newMeRequest(session, new GraphUserCallback()
					{
						...
						public void onCompleted(GraphUser user, Response response)
							...
							if (user != null)
								...
								// NAME
								userSP.edit().putString("name", user.getName().toString()).commit();

								// EMAIL
								userSP.edit().putString("email", getEmail(getBaseContext())).commit();

								// AGE
								Calendar c = Calendar.getInstance();

								userSP.edit().putInt("age", c.get(Calendar.YEAR) - Integer.parseInt(user.getBirthday().substring(user.getBirthday().length() - 4)))
										.commit();
